...
mod kernel_mode {
    ...
    // `FILE_INFORMATION_CLASS` values from `ntifs.h` that are not generated by bindgen.
    // These are used with `FltSetInformationFile` and IRP_MJ_SET_INFORMATION to specify
    // the type of information being set for a file. Typed as `i32` to match the
    // `FileInformationClass` field type in the generated structures.
    //
    ...
    pub const FILE_RENAME_INFORMATION_BYPASS_ACCESS_CHECK: i32 = 71;
    /// Extended link information (supports additional flags).
    pub const FILE_LINK_INFORMATION_EX: i32 = 72;
    /// Extended rename information that bypasses access checks (kernel mode only).
    pub const FILE_RENAME_INFORMATION_EX_BYPASS_ACCESS_CHECK: i32 = 73;
    /// Link information that bypasses access checks (kernel mode only).
    pub const FILE_LINK_INFORMATION_BYPASS_ACCESS_CHECK: i32 = 74;
    /// Extended link information that bypasses access checks (kernel mode only).
    pub const FILE_LINK_INFORMATION_EX_BYPASS_ACCESS_CHECK: i32 = 75;
}
