...
* `taplo-cli`: `cargo install --git https://github.com/tamasfe/taplo --rev b673b44d taplo-cli --locked --force`
  * This pinned revision is required: released versions of `taplo` do not yet support the field-level `keys` matching that `taplo.toml` uses to keep positional `args`/`dependencies` arrays from being alphabetized, and running one will reorder the arguments of the `cargo-make` tasks in `Makefile.toml` and `rust-driver-makefile.toml`.
